...
You can also compile Solidity code via Ethereum clients such as Geth, and on legacy deployments via Parity or OpenEthereum-compatible clients, using the JSON-RPC method [eth_compileSolidity](https://github.com/ethereum/wiki/wiki/JSON-RPC#eth_compileSolidity) which is also supported in Web3j. However, the Solidity compiler must be
